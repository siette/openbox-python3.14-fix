...
class Menu:
    ...
    def merge(self, other):
        for submenu in other.Submenus:
            self.addSubmenu(submenu)
        self.MenuEntries.extend(other.MenuEntries)
        self.Rules.extend(other.Rules)
        self.AppDirs.extend(other.AppDirs)
        self.DirectoryDirs.extend(other.DirectoryDirs)
...
# CORREGIT: ara retorna correctament en mode recursiu
    def getMenuEntry(self, desktopfileid, deep=False):
        ...
        if deep:
            for submenu in self.Submenus:
                result = submenu.getMenuEntry(desktopfileid, deep)
                if result:
                    return result
...
class Rule:
    ...
    def fromFilename(cls, type, filename):
        tree = ast.Expression(
            body=ast.Compare(
                left=ast.Constant(value=filename),
                ops=[ast.Eq()],
...
class XMLMenuBuilder(object):
    ...
    def parse_rule(self, node):
        ...
        if expr:
            ...
            tree.body = ast.Constant(value=False)
        ast.fix_missing_locations(tree)
    ...
    def parse_rule_node(self, node):
        ...
        if tag == 'Or':
            ...
            category = node.text
            return ast.Compare(
                    left=ast.Constant(value=category),
                ops=[ast.In()],
                comparators=[ast.Attribute(
                    value=ast.Name(id='menuentry', ctx=ast.Load()),
                    attr='Categories',
